...
func newWinUser(db *gorm.DB, opts ...gen.DOOption) winUser {
	...
	_winUser.PackageName = field.NewString(tableName, "package_name")
	_winUser.GpsAdid = field.NewString(tableName, "gps_adid")
	_winUser.AdjustAdid = field.NewString(tableName, "adjust_adid")
...
type winUser struct {
	...
	RoleID            field.Int64  // 角色:1=游客,8=普通用户,12=主播,16=超管用户,20=代理
...
func (w *winUser) updateTableName(table string) *winUser {
	...
	w.GpsAdid = field.NewString(table, "gps_adid")
...
func (w *winUser) fillFieldMap() {
	w.fieldMap = make(map[string]field.Expr, 93)
	w.fieldMap["id"] = w.ID
